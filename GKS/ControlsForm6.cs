...
namespace GKS
{
    class ControlsForm6
    {
        ...
        public ControlsForm6(Panel panel, RichTextBox newGroupList)
        {
            mainPanel = panel;
            this.newGroupList = newGroupList;
        }

        public void ClearAndStart(string[][] mainArray, string[][] finalResult)
        {
            Calculation6 c6 = new Calculation6(mainArray, finalResult);
            c6.StartCalculation(out result);
        ...
        {
            newGroupList.Text = "";
            for (int i = 0; i < result[currentGroup - 1].Length; i++)
            {
                newGroupList.Text += "M" + (i + 1) + ": {";
                for (int j = 0; j < result[currentGroup - 1][i].Length; j++)
                {
                    newGroupList.Text += result[currentGroup - 1][i][j] + ", ";
                }
